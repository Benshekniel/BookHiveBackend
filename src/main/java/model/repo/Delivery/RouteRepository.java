// RouteRepository.java - Fixed Complete Version
...
/**
 * Complete RouteRepository interface for Route entity operations
 * Fixed to work without dependencies on entities that may not exist
 */
@Repository
public interface RouteRepository extends JpaRepository<Route, Long> {

    // ================================
    // SAFE AGENT-ROUTE RELATIONSHIPS
    // ================================

    /**
     * Get agents by route - simplified version that doesn't depend on RouteAssignment table
     * This returns agents from the same hub as the route
     */
    @Query("SELECT a FROM Agent a WHERE a.hubId = (SELECT r.hubId FROM Route r WHERE r.routeId = :routeId)")
    List<Agent> findByRouteId(@Param("routeId") Long routeId);

    /**
     * Alternative: Get agents by hub ID directly (safer)
     */
    @Query("SELECT a FROM Agent a WHERE a.hubId = :hubId")
    List<Agent> findAgentsByHubId(@Param("hubId") Long hubId);

    /**
     * Get available agents for a route (from same hub)
     */
    @Query("SELECT a FROM Agent a WHERE a.hubId = (SELECT r.hubId FROM Route r WHERE r.routeId = :routeId) " +
            "AND a.availabilityStatus = 'AVAILABLE'")
    List<Agent> findAvailableAgentsForRoute(@Param("routeId") Long routeId);
    ...
    Optional<Route> findRouteWithBoundaries(@Param("routeId") Long routeId);
    ...
    // ================================
    // PRIORITY AND OPTIMIZATION QUERIES
    // ================================
